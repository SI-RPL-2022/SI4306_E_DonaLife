                ...
                <form id="form_input">
                        ...
                        <div class="col-lg-12 mb-5">
                            ...
                            <select class="form-control" name="campaign" id="campaign">
                                ...
                                @foreach($campaign as $option)
                                    <option value="{{$option->id}}" {{$option->id == $data->campaign_uang_id ? 'selected' :''}}>{{$option->judul}}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="row">
                        <div class="min-w-150px pt-10 mt-10 text-end">
                            @if ($data->id)
                            <button id="tombol_simpan" onclick="handle_upload('#tombol_simpan','#form_input','{{route('admin.berita-campaignUang.update',$data->id)}}','PATCH');" class="btn btn-sm btn-primary">Simpan</button>
                            @else
                            <button id="tombol_simpan" onclick="handle_upload('#tombol_simpan','#form_input','{{route('admin.berita-campaignUang.store')}}','POST');" class="btn btn-sm btn-primary">Simpan</button>
                            @endif
                        </div>
                    </div>
                </form>
